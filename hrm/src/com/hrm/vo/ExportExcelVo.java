...
public class ExportExcelVo implements BaseVo {
	...
	public Request execute(Request request) throws Exception {
		...
		**/
		String th[] = {"品名","菜名","肉","销售单价","数量","销售份数","销售金额","进货均价","上月库存","本月进货量","月末实际库存","月末理论库存","损耗率（%）"};
		...
		//创建标题
		t.createTitle(sheet,boldCell,"菜品月销售报表 （"+request.getParamsMap().get("{0}")+" ~ "+request.getParamsMap().get("{1}")+"）");
		//创建表头
		...
		for (MaterialList materialList : type) {
			...
			**/
			if(temp.size()>0){
				System.out.println("start:"+start+" temp.size(): "+temp.size()+"  all:"+(start+temp.size()));
				...
				for (int i = 0; i < temp.size(); i++) {
					...
					System.out.println("start+i+1:"+(start+i)+"  "+map.get("footname").toString());
					sheet.addCell(new Label(1, start+i, map.get("footname").toString(), normalContentCell));
					sheet.addCell(new Label(2, start+i, materialList.getParamsname()+" "+map.get("materialamount").toString()+map.get("unitname").toString(), normalContentCell));
					sheet.addCell(new Label(3, start+i, map.get("footprice").toString(), normalContentCell));
					sheet.addCell(new Label(4, start+i, map.get("materialamount").toString(), normalContentCell));
					sheet.addCell(new Label(5, start+i, map.get("materialsum").toString(), normalContentCell));
					sheet.addCell(new Label(6, start+i, map.get("materialcostsum").toString(), normalContentCell));
					materialamountsum += Float.valueOf(map.get("materialamount").toString());
					materialsumsum += Float.valueOf(map.get("materialsum").toString());
				...
				sheet.addCell(new Label(4, start+temp.size(), materialamountsum+"", boldCell));
