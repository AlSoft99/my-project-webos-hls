...
//		height : height-500,
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
//				parent: "select new map(a.id as id,a.id || '[' || b.paramsname || ']' as text,b.paramsname as qtip,a.manguser as manguser) from StoreInputInfo a,ParamsList b,StoreParamsInfo c where b.typeid='CONFIRM_STATUS' and a.confirmstatus='1' and b.paramscode=a.confirmstatus and a.applydate>'{1}' and a.applydate<'{2}' and a.manguser='"+Ext.getDom("user_id").value+"' and c.outuser='"+Ext.getDom("user_id").value+"' and c.stocklevel='1' and a.preprice>=c.min and a.preprice<c.max",
...
/*    var comboBoxType = comboBoxList.comboBoxSql("select id,typename from StoreType ","","");
...
/*  	comboBoxType.on("select",function(obj,option){
  	...
  	var grid = new Ext.grid.GridPanel({
    	...
    	tbar:[{
    		ref: '../addBtn',
            text:'确认货物申购',
            tooltip:'确认货物已申购, 申购货物将不能回滚, 请确认货物已申购',
            iconCls:'save',
            disabled: true,
            handler:function(){
            	Ext.MessageBox.show({
					title:"警告提示",
					msg:"请确认采购单单号为<font color='red'>"+currentId+"</font>的单子已申购,确定后将不能回滚,并且发送到财务科!!",
					...
					fn:function(btn){
						if(btn=="ok"){
			                ...
			                Ext.Ajax.request({
				        		url:"storeInputInfoVo.do?action=confirm",
				        		params:{
				        			id:currentId
				        		},
				        		success:function(action){
...
//    		comboBoxList.goodsList.getStore().load();
	...
	new Ext.Panel({
        ...
        collapsible:true,
        hideBorders:true,
        renderTo: 'stock_manager_confirm_name',
        width : 350,
        height : height-100,
        items:[
        	tree_role,
    ...
    new Ext.Panel({
        ...
        collapsible:true,
        width : width-595,
        renderTo: 'stock_manager_confirm_list',
        height : height-100,
        items:[grid]
    });
