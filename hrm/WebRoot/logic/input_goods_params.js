Ext.onReady(function() {
	...
	var tree_menu = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
				parent: "select distinct new map(id.userlist as id,id.userlist as text,id.userlist as qtip) from OutUserList where id.tag='2'"
...
//			grid.addBtn.setDisabled(true);
		...
		Ext.getCmp("input_goods_params_role_code").setValue("");
		Ext.getCmp("input_goods_params_save").setDisabled(currentId=="0");
		Ext.getCmp("input_goods_params_query").setDisabled(currentId=="0");
	});
	/**
	 * 右键菜单
	 */
			  	...
			  	listeners:{
			  		select:function(obj,option){
			  			...
			  			form.getComponent(1).getStore().load({
			  				params:{sql:"select userId,userName from UserInfo where userId not in (select id.outuser from OutUserList where id.tag='2') and roleInfo.roleCode='"+value+"'"}
			  			});
	...
	var contextmenu = new Ext.menu.Menu({
  		items:[{
    		...
    		handler:function(event,mouse){//bcescvr,110400
    			...
    			form.getComponent(1).getStore().load({
	  				params:{sql:"select userId,userName from UserInfo where userId not in (select id.outuser from OutUserList where id.tag='2')"}
	  			});
	  			form.getComponent(1).setValue("");
...
//	propertySource = {姓名:'官建明',工作天数:'6',基本工资:'20000.0',上岗工资:'30000.0',福利补贴:'1000.0',个人所得税:'2000.0',公积金:'2200.0',养老保险金:'2400.0',医疗保险金:'2600.0',综合保险金:'1000.0',其他福利保险:'0.0'};
    	...
    	loadGrid:function(){
    		...
    		form1.getComponent(0).multiselects[0].store.load({
  			...
  			form1.getComponent(0).multiselects[1].store.load({
  				params:{sql:"select a.id.outuser,b.userName from OutUserList a,UserInfo b where a.id.tag='2' and a.id.outuser=b.userId and a.id.userlist='"+currentId+"'"}
  			});
        ...
        items:[{
            ...
            multiselects: [{
		        ...
		        tbar:["-",new Ext.form.ComboBox({
			  		name: "roleCode",
			  		id:"input_goods_params_role_code",
				  	store: new Ext.data.Store({
			            proxy:new Ext.data.HttpProxy({url:"optionServlet.do?option=roleInfo"}),
			            autoLoad:true,
						reader:new Ext.data.ArrayReader({},[
					...
					iconCls:"icon-grid",
					disabled: true,
					id:"input_goods_params_query",
					handler:function(){
						var hql = "select userId,userName from UserInfo where userId not in (select id.outuser from OutUserList where id.tag='2')";
						if(Ext.getCmp("input_goods_params_role_code").getValue()!=""){
							hql += " and roleInfo.roleCode='"+Ext.getCmp("input_goods_params_role_code").getValue()+"'";
						}
						loading.loadMask().show();
						form1.getComponent(0).multiselects[0].store.load({
			  				params:{sql:hql}
                ...
                store: new Ext.data.Store({
		            ...
		            baseParams:{
	            		sql:"select a.id.outuser,b.userName from OutUserList a,UserInfo b where a.id.tag='2' and a.id.outuser=b.userId and a.id.userlist='"+currentId+"'"
	            	},
		            autoLoad:true,
					reader:new Ext.data.ArrayReader({},[
					    {name:"value"},
	...
	new Ext.Panel({
        title: '仓库查询',
        collapsible:true,
        renderTo: 'input_goods_params_name',
        width : 300,
        height : height-100,
        items:[
	        new Ext.TabPanel({
    ...
    new Ext.Panel({
        ...
        collapsible:true,
        width : width-545,
        renderTo: 'input_goods_params_list',
        height : height-100,
        items:[form1]
    });
