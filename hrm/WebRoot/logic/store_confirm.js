Ext.onReady(function() {
	...
	var tree_role = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
				parent: "select new map(a.id as id,a.id || '[' || b.paramsname || ']' as text,b.paramsname as qtip,a.recuser as recuser) from StoreOutputInfo a,ParamsList b where b.typeid='STORE_STATUS' and a.status='1' and b.paramscode=a.status and outdate>'{1}' and outdate<'{2}' and recuser='"+document.getElementById("user_id").value+"'",
	...
	var cm = new Ext.grid.ColumnModel([
  	...
  	var grid = new Ext.grid.GridPanel({
    	...
    	tbar:[{
    		ref: '../addBtn',
            text:'确认货物出仓',
            tooltip:'确认货物已出仓, 出仓货物将不能回滚, 请确认货物已出仓',
            iconCls:'save',
            disabled: true,
            handler:function(){
            	Ext.MessageBox.show({
					title:"警告提示",
					msg:"请确认出仓单为<font color='red'>"+currentId+"</font>是否货物已出仓,确定后将不能回滚!!",
					...
					fn:function(btn){
						if(btn=="ok"){
			                ...
			                Ext.Ajax.request({
				        		url:"storeOutputInfoVo.do?action=confirm",
				        		params:{
				        			id:currentId
				        		},
				        		success:function(action){
