Ext.onReady(function() {
		        ...
		        width:80,
			  	...
			  	listeners:{
			  		select:function(obj,option){
			  			...
			  			toolbar.getComponent(3).getStore().load({
			  				params:{option:"userInfo",change:roleCode}
			  			});
			  			toolbar.getComponent(3).setValue("");
			  		}
			  	}
			}),"用户:",
			new Ext.form.ComboBox({
				...
				handler:function(event,mouse){
					tree_menu.loader.baseParams.parent = "select new map(a.id as id,a.typename as text,a.typedesc as qtip) from StoreType a,OutUserList b where a.storelist=b.id.userlist and b.id.tag='2' and b.id.outuser='"+toolbar.getComponent(3).getValue()+"'"
					tree_menu.getRootNode().setText("参数种类 ["+toolbar.getComponent(3).getRawValue()+"]");
					tree_menu.getRootNode().reload();
					currentId = "";
					listenerEvent.loadGrid();
				}
	...
	var toolbar = new Ext.Toolbar({
		items:[
			...
			comboBoxName,
			{
				...
				tooltip:'请选择仓库查询',
				handler:function(event,mouse){
					tree_menu.loader.baseParams.parent = "select new map(a.id as id,a.typename as text,a.typedesc as qtip) from StoreType a where a.storelist='"+toolbar.getComponent(1).getValue()+"'";
	...
	var tree_menu = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
				parent: "select new map(a.id as id,a.typename as text,a.typedesc as qtip) from StoreType a,OutUserList b where a.storelist=b.id.userlist and b.id.tag='2' and  a.storelist='"+toolbar.getComponent(1).getValue()+"'"
...
//	propertySource = {姓名:'官建明',工作天数:'6',基本工资:'20000.0',上岗工资:'30000.0',福利补贴:'1000.0',个人所得税:'2000.0',公积金:'2200.0',养老保险金:'2400.0',医疗保险金:'2600.0',综合保险金:'1000.0',其他福利保险:'0.0'};
	...
	new Ext.Panel({
        title: '货物参数查询',
        collapsible:true,
        renderTo: 'input_store_params_name',
        width : 300,
        height : height-100,
        items:[
	        new Ext.TabPanel({
    ...
    new Ext.Panel({
        ...
        collapsible:true,
        width : width-545,
        renderTo: 'input_store_params_list',
        height : height-100,
        items:[grid]
    });
