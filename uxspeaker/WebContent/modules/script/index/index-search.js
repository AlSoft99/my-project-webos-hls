var index = {
	queryArticle: function(current){
		...
		var typename = $("#index").attr("search");
		var option = {start:current,row:rownumber,logout:true,sql:"SQL6",where:"and (d.tagname like '%"+typename+"%' or a.title like '%"+typename+"%' or a.content like '%"+typename+"%' )"};
		$("#index-content").empty();
