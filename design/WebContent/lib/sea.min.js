...
(function(a,g,e){function k(a){a=a.match(/.*(?=\/.*$)/);return(a?a[0]:".")+"/"}function i(m){m=m.replace(/([^:\/])\/+/g,"$1/");if(m.indexOf(".")===-1)return m;for(var d=m.split("/"),b=[],h,f=0,c=d.length;f<c;f++)h=d[f],h===".."?(b.length===0&&a.error({message:"invalid path: "+m,type:"error"}),b.pop()):h!=="."&&b.push(h);return b.join("/")}function b(a){a=i(a);/#$/.test(a)?a=a.slice(0,-1):a.indexOf("?")===-1&&!/\.(?:css|js)$/.test(a)&&(a+=".js");return a}function c(a){function d(a,m){var h=a[m];b&&
b.hasOwnProperty(h)&&(a[m]=b[h],f=true)}var b=o.alias;if(!b)return a;var a=a.split("/"),h=a.length-1,f=false;d(a,0);!f&&h&&d(a,h);return a.join("/")}function j(d,b){b=b||o.map||[];if(!b.length)return d;var h=[];a.forEach(b,function(a){a&&a.length>1&&(a[2]===-1?h.push([a[0],a[1]]):d=d.replace(a[0],a[1]))});h.length&&(d=j(d,h));return d}function l(a){return a.replace(/^(\w+:\/\/[^/]*)\/?.*$/,"$1")}function d(d,h,f){f||(d=c(d));h=h||p;q(d)||(d.indexOf("./")===0||d.indexOf("../")===0?(d=d.replace(/^\.\//,
...
-1||a.indexOf("//")===0}var o=g.config,e=e.location,p=e.protocol+"//"+e.host+function(a){a.charAt(0)!=="/"&&(a="/"+a);return a}(e.pathname);p.indexOf("\\")!==-1&&(p=p.replace(/\\/g,"/"));var n=g.memoizedMods;a.dirname=k;a.parseAlias=c;a.id2Uri=d;a.ids2Uris=h;a.memoize=function(a,b,f){var c;c=a?d(a,b,true):b;f.id=c;f.dependencies=h(f.dependencies,c);n[c]=f;a&&b!==c&&(a=n[b])&&r(a.dependencies,f.dependencies)};a.setReadyState=function(d){a.forEach(d,function(a){if(n[a])n[a].ready=true})};a.getUnReadyUris=
function(d){return a.filter(d,function(a){a=n[a];return!a||!a.ready})};a.removeCyclicWaitingUris=function(d,b){return a.filter(b,function(a){return!f(n[a],d)})};a.isAbsolutePath=q;if(o.debug)a.realpath=i,a.normalize=b,a.getHost=l})(seajs._util,seajs._data,this);
