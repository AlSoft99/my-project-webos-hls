Ext.onReady(function() {
	        ...
	        width:80,
		  	...
		  	listeners:{
		  		select:function(obj,option){
		  			...
		  			toolbar.getComponent(10).getStore().load({
		  				params:{option:"userInfo",change:roleCode}
		  			});
		  			toolbar.getComponent(10).setValue("");
		  		}
		  	}
		}),"申购人:",
		new Ext.form.ComboBox({
			...
			handler : function(event, mouse) {
				...
				var applyuser = toolbar.getComponent(10).getValue();
				if (applyuser != "") {
					where = " and a.applyuser='" + applyuser + "'"
				} else {
					where = "";
				}
	...
	var grid_info = new Ext.grid.GridPanel({
		...
		columns : [new Ext.grid.RowNumberer(), {
			...
			summaryType : 'count',
			summaryRenderer : function(v, params, data) {
				return ((v === 0 || v > 1) ? '(申购状态' + v + ' 个)' : '(申购状态1 个)');
			},
			sortable : true
		}, {
			xtype : 'numbercolumn',
...
//			xtype : 'numbercolumn',
	...
	listenerEvent.loadGrid();
	layout.render("all_stock_list_div");
