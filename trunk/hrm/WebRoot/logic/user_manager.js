Ext.onReady(function(){
	...
	var sm = new Ext.grid.CheckboxSelectionModel({
		listeners: {
            rowselect: function(sm, row, rec) {
                user_manager_form.getForm().loadRecord(rec);
            }
        }
	...
	var resetForm = function(){
	                    	...
	                    	if(basicForm.isValid()){
	                    		basicForm.submit({
	                    			success:function(form,action){
	                    				...
	                    				resetForm();
	                    				user_manager_form.getForm().reset();
	                    				ds.reload();
	                    				win.hide();
	                    			},params:{
	                    				action:"insert"
  	...
  	grid.render();
  	var user_manager_height = Ext.getBody().getHeight();
  	var user_manager_form = new Ext.form.FormPanel({
	  	defaultType:"textfield",
	  	labelAlign:"right",
	  	url:"userInfoVo.do",
	  	labelWidth:80,
	  	height:user_manager_height,
	  	frame:true,
	  	width:300,
	  	items:[{
	    	name:"user_id",
	...
	new Ext.Panel({
        title: '用户详细信息',
        collapsible:true,
        renderTo: 'user_manager_panel',
        height:user_manager_height,
        width:300,
        draggable:true,
        items:[user_manager_form],
        tbar:[{
            text:'修改用户',
            tooltip:'修改一条用户记录',
            iconCls:'option',
            handler:function(){
            	...
            	var basicForm = user_manager_form.getForm();
            	if(basicForm.isValid()){
            		basicForm.submit({
            			success:function(form,action){
            				Ext.MessageBox.show({
