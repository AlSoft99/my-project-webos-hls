Ext.onReady(function () {
    ...
    var tree_menu = new Ext.tree.TreePanel({
        ...
        loader: new Ext.tree.TreeLoader({
            ...
            baseParams: {
                ...
                parent: "select new map(id as id,id||' ['||orderdesc||']' as text,orderdesc as qtip) from OrderMaterialStoreInfo where outuser='" + document.getElementById("user_id").value + "' and outdate>'{1}' and outdate<'{2}'"
    ...
    var grid = new Ext.grid.GridPanel({
		...
		{name: "returnnumber", type: 'int'}
                ...
                Ext.Ajax.request({
                    url: "orderMaterialStoreVo.do?action=insertRecord",
                    params: record,
                    success: function (action) {
                ...
                Ext.MessageBox.show({
                    ...
                    fn: function (btn) {
                        if (btn == "ok") {
                            ...
                            Ext.Ajax.request({
                                url: "orderMaterialStoreVo.do?action=deleteRecord",
                                params: {
                                    id: id,
                                    outid: currentId
                                },
    ...
    var listenerEvent = {
        update: function (updateObj, jsonRecord) {
            ...
            loading.loadMask().show();
            Ext.Ajax.request({
                url: "orderMaterialStoreVo.do?action=updateRecord",
                params: recordData,
                success: function (action) {
                    var json = eval("(" + action.responseText + ")");
                    if (json.failure) {
