...
//		height : height-500,
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
				parent: "select new map(a.id as id,a.id || '[' || b.paramsname || ']' as text,b.paramsname as qtip,a.manguser as manguser) from StoreInputInfo a,ParamsList b where b.typeid='CONFIRM_STATUS' and a.confirmstatus='4' and b.paramscode=a.confirmstatus and a.applydate>'{1}' and a.applydate<'{2}' and a.accouser='"+Ext.getDom("user_id").value+"'",
	...
	var cm = new Ext.grid.ColumnModel([
  	...
  	var grid = new Ext.grid.GridPanel({
    	...
    	tbar:[{
    		ref: '../addBtn',
            text:'确认货物结算',
            tooltip:'确认货物已结算, 结算货物将不能回滚, 请确认货物已结算',
            iconCls:'save',
            disabled: true,
            handler:function(){
            	var form = new Ext.form.FormPanel({
			        ...
			        buttons: [{
			            ...
			            handler	 : function(){
			            	...
			            	if(basicForm.isValid()){
			            		...
			            		Ext.MessageBox.show({
									title:"警告提示",
									msg:"请确认采购单单号为<font color='red'>"+currentId+"</font>的单子已结算,确定后将不能回滚,并且发送到仓库管理员!!",
									...
									fn:function(btn,text){
										if(btn=="ok"){
							                ...
							                Ext.Ajax.request({
								        		url:"storeInputInfoVo.do?action=clearfinance",
								        		params:{
								        			id:currentId,
								        			autprice:form.getComponent(0).getValue()
								        		},
								        		success:function(action){
...
//    		comboBoxList.goodsList.getStore().load();
	...
	new Ext.Panel({
        ...
        collapsible:true,
        hideBorders:true,
        renderTo: 'stock_finance_clear_name',
        width : 350,
        height : height-100,
        items:[
        	tree_role,
    ...
    new Ext.Panel({
        ...
        collapsible:true,
        width : width-595,
        renderTo: 'stock_finance_clear_list',
        height : height-100,
        items:[grid]
    });
