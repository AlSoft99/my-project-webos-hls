Ext.onReady(function() {
	...
	var tree_menu = new Ext.tree.TreePanel({
		...
		loader : new Ext.tree.TreeLoader({
			...
			baseParams :{
				parent: "select new map(id as id,typename as text,typedesc as qtip) from MaterialTypeKtv"
		...
		if(node.leaf){
			...
			/*ds.load({
		  		params:{
		  			...
		  			action:"hql",
					type:"entity",
					sql:"from MaterialListKtv where typeid='"+currentId+"'"
		  		}
		  	});*/
		}else{
...
//							   		tree_menu.remove(node_menu);
	...
	var unit = comboBoxList.comboBoxSql("select paramscode,paramsname from ParamsList where typeid='KTV_UNIT'","","");
  	...
  	var grid = new Ext.grid.GridPanel({
    	...
    	tbar:[{
            ...
            handler:function(){
                ...
                Ext.Ajax.request({
	        		url:"materialTypeKtvVo.do?action=insertRecord",
	        		params:record,
	        		success:function(action){
...
//							loading.loadMask().show();
							Ext.Ajax.request({
				        		url:"materialTypeKtvVo.do?action=deleteRecord",
				        		params:{
				        			id:id,
				        			typeid:currentId
				        		},
...
//	propertySource = {姓名:'官建明',工作天数:'6',基本工资:'20000.0',上岗工资:'30000.0',福利补贴:'1000.0',个人所得税:'2000.0',公积金:'2200.0',养老保险金:'2400.0',医疗保险金:'2600.0',综合保险金:'1000.0',其他福利保险:'0.0'};
	...
	var listenerEvent = {
    	update:function(updateObj,jsonRecord){
    		...
    		loading.loadMask().show();
    		Ext.Ajax.request({
    			url:"materialTypeKtvVo.do?action=updateRecord",
    			params:recordData,
    			success:function(o){
    	...
    	loadGrid:function(){
    		ds.baseParams.sql = "from MaterialListKtv where typeid='"+currentId+"'";
	...
	new Ext.Panel({
        title: '种类',
        collapsible:true,
        renderTo: 'ktv_material_params_info_name',
        width : 250,
        height : height-100,
        items:[
	        new Ext.TabPanel({
    ...
    new Ext.Panel({
        ...
        collapsible:true,
        width : width-495,
        renderTo: 'ktv_material_params_info_list',
        height : height-100,
        items:[grid]
    });
