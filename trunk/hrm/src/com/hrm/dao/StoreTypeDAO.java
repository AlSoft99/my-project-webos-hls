 ...
 * 
 * @see com.hrm.entity.StoreType
 * @author MyEclipse Persistence Tools
 */

public class StoreTypeDAO extends HibernateDaoSupport {
	private static final Log log = LogFactory.getLog(StoreTypeDAO.class);
	// property constants
	...
	public StoreType findById(java.lang.String id) {
		...
		try {
			StoreType instance = (StoreType) getHibernateTemplate().get(
					"com.hrm.entity.StoreType", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByStorelist(Object storelist) {
		return findByProperty(STORELIST, storelist);
	}
	...
	public List findAll() {
		...
		try {
			String queryString = "from StoreType";
	...
	public StoreType merge(StoreType detachedInstance) {
		...
		try {
			StoreType result = (StoreType) getHibernateTemplate().merge(
					detachedInstance);
	...
	public static StoreTypeDAO getFromApplicationContext(ApplicationContext ctx) {
		return (StoreTypeDAO) ctx.getBean("StoreTypeDAO");
	}
}